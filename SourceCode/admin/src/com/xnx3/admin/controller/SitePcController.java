...
//	        	//从内存中取模版变量内容
//	        	String content = com.xnx3.admin.G.templateVarMap.get(site.getTemplateName()).get(templateVarName);
//	        	
//	        	//讲用户保存的跟内存中的模版变量内容比较，是否一样，若不一样，要将当前的保存
//	        	if(!(content.replaceAll("\r|\n|\t", "").equals(templateVarText.replaceAll("\r|\n|\t", "")))){
//	        		//不一样，进行保存
//	        		com.xnx3.admin.entity.TemplateVar tv = sqlService.findAloneBySqlQuery("SELECT * FROM template_var WHERE template_name = '"+site.getTemplateName()+"' AND var_name = '"+templateVarName+"'", com.xnx3.admin.entity.TemplateVar.class);
...
//	}
	 ...
	 * 首页，点击左侧菜单的高级设置生效
	 */
	@RequestMapping("popupGaoJiSet.do")
