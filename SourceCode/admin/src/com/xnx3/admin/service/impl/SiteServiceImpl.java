...
import com.xnx3.admin.cache.TemplateCMS;
import com.xnx3.admin.cache.pc.IndexAboutUs;
...
import com.xnx3.admin.vo.IndexVO;
import com.xnx3.admin.vo.SiteColumnTreeVO;
import com.xnx3.admin.vo.SiteRemainHintVO;
...
import com.xnx3.admin.vo.bean.TemplateCommon;
...
//		TemplatePageListVO templatePageListVO = templateService.getTemplatePageListByCache(request);
		...
		Map<String, List<com.xnx3.admin.bean.News>> columnTreeNewsMap = new HashMap<String, List<com.xnx3.admin.bean.News>>();
		for (Map.Entry<String, SiteColumnTreeVO> entry : columnTreeMap.entrySet()) {
			...
			if(sct.getList().size() > 0){
				//有子栏目，才会对其进行数据汇总
				columnTreeNewsMap.put(sct.getSiteColumn().getCodeName(), new ArrayList<com.xnx3.admin.bean.News>());
				...
				for (int i = 0; i < sct.getList().size(); i++) {
					...
					List<com.xnx3.admin.bean.News> nList = new ArrayList<com.xnx3.admin.bean.News>(); 
					List<News> oList = columnNewsMap.get(subSct.getSiteColumn().getCodeName());
					for (int j = 0; j < oList.size(); j++) {
						com.xnx3.admin.bean.News n = new com.xnx3.admin.bean.News();
...
//		System.out.println("paixuqian:"+columnTreeMap.size());
		...
		for (Map.Entry<String, SiteColumnTreeVO> entry : columnTreeMap.entrySet()) {
			SiteColumnTreeVO sct = entry.getValue();
			if(sct.getList().size() > 0){
				List<com.xnx3.admin.bean.News> nList = columnTreeNewsMap.get(sct.getSiteColumn().getCodeName());
...
//		Map<String, String> varMap = Func.getUserBeanForShiroSession().getTemplateVarDataMapForOriginal();
	...
	public TemplateCommon getTemplateCommonHtml(Site site, String title, Model model) {
		...
		String headHtml = FileUtil.read(Global.getProjectPath()+"/WEB-INF/template/"+gh.templateId+"/common/head.html");
		...
		String topHtml = FileUtil.read(Global.getProjectPath()+"/WEB-INF/template/"+gh.templateId+"/common/top.html");
		...
		String footHtml = FileUtil.read(Global.getProjectPath()+"/WEB-INF/template/"+gh.templateId+"/common/foot.html");
		...
		if(site.getId() > 0){
			...
			if(s.getId() > 0){
				...
				if(s.getClient() - Site.CLIENT_CMS == 0){
					...
					new com.xnx3.admin.cache.Site().carousel(carouselList,site);	//创建轮播图缓存数据
...
//						lxSC.setClient(SiteColumn.CLIENT_PC);
						...
						//WAP
						columnList.add(sc);
						new com.xnx3.admin.cache.Site().siteColumn(columnList,site); //siteColumn.js
						new com.xnx3.admin.cache.Site().siteColumnRank(site, sc.getId()+"");	//siteColumnRank.js
					}
