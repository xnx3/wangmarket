				...
				<c:choose>
					<c:when test="${!news.titlepic.isEmpty() && fn:length(news.titlepic) > 1}">
						<c:choose>
						    ...
						    <c:otherwise>
								<a href="<%=OSSUtil.url %>site/${news.siteid }/news/${news.titlepic }" target="_black"><img src="<%=OSSUtil.url %>site/${news.siteid }/news/${news.titlepic }?x-oss-process=image/resize,h_100" height="50" /></a>
						    </c:otherwise>
						</c:choose>
					</c:when>
					<c:otherwise>
