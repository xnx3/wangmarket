		...
		if(version.length > 0 && oldVersion != version){
			...
			layer.open({
				...
				,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300; text-align:left;">'
					...
					+'<li>网站增加全局变量功能，网站可自定义全局变量，使用{var.xxx}在模板中随意调用，在网站管理后台的模板管理下即可看到。</li>'
					+'<li>增加文件管理插件，网站管理后台中，可以通过此插件，随意新建文件夹、上传文件、在线编辑html、txt等文件。</li>'
					+'<li>网站栏目，增加 SEO 设置项，可设置 keywords、 description ，并且模板中，可以使用栏目标签 {siteColumn.keywords} 、 {siteColumn.description} 来动态调用</li>'
					+'<li>增加 robots.txt 插件</li>'
					+'<li>加载中、成功、失败等信息提示，由layui改为我们自家的 msg.js</li>'
					+'<li>增加 SqlCacheService 查询的数据先从缓存（内存或redis）中取，若没有，再从数据库中取，取出后加入缓存。</li>'
					+'<li>AttachmentUtil 增加 文件夹创建、获取文件大小 的方法。</li>'
					+'<li>修复冻结的网站，重启后未冻结的问题</li>'
					+'<li>优化 SessionUtil ，可以根据userid，来将该用户所有的session删除，强制踢此用户下线</li>'
					+'<li>修复网站栏目中，编辑时，显示选项卡中，子栏目列表、内容管理中，设置为隐藏后，导出模板， 再新网站中将模板导入时，这两项都是显示 的问题</li>'
					+'<li>优化制作模板时，若没有类型为首页的模板，会一直显示生成中的问题。改为生成的首页会显示友好提示。</li>'
					+'<li>增加华为云短信通道（插件）</li>'
					+'<li>修复上次升级遗漏，网站管理后台登录成功后，顶部的title还是登录的问题</li>'
					+'<li>网站创建后，选择模版，模版列表图片增加缩放，提高加载速度</li>'
					+'<li>去除 WEI UI 库</li>'
					+'<li>修复刚创建的网站，不使用现有模版，自己开发模版时，增加或修改了模版变量后，在模版页面编辑时，可用模版变量提示未变过来的问题。</li>'
					+'<li>当登录时，提示系统中没有存储验证码时，提示文字增加三个字，请重试</li>'
					+'<li>代理后台去掉站币日志菜单</li>'
