		...
		if(version.length > 0 && oldVersion != version){
			...
			layer.open({
				...
				,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300; text-align:left;">'
					...
					+'<li>增加网站api接口，可以通过api接口获取栏目信息、文章信息、全局变量信息等，自由进行各种定制</li>'
					+'<li>全面优化升级底层支持包版本。</li>'
					+'<li>网站访问增加缓存层，提高响应速度、安全保护，有效提高响应速度、云存储的读次数，降低如阿里云OSS在晚上服务不稳定造成访问卡顿情况。</li>'
					+'<li>总管理后台-网站管理，网站列表增加排序</li>'
					+'<li>总管理后台-网站管理，增加对网站的冻结及解冻操作</li>'
					+'<li>全面升级底层支持包版本</li>'
