		...
		if(version.length > 0 && oldVersion != version){
			...
			layer.open({
				...
				,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300; text-align:left;">'
					...
					+'<li>CMS模式网站，模版页面增加纯代码模式，同织梦CMS、帝国CMS的编辑方式。现在模版页面编辑方式共有两种，可视化编辑模式、代码模式。</li>'
					+'<li>增加 newsSearch 站内文章搜索的功能插件，让网站快速拥有整站搜索功能！</li>'
					+'<li>代理后台增加公告展示、以及代理信息自行修改功能。</li>'
					+'<li>CMS模式网站管理后台，栏目管理中，栏目添加/修改，增加tab选项卡，将基本设置、高级设置区分开。普通使用时，高级设置可无需看即可设置栏目常用属性。</li>'
					+'<li>网站管理后台，增加栏目内容排序功能。当栏目类型为新闻列表、图文列表时，在高级设置中，可设置栏目内信息的排序方式，默认是按时间倒序，增加按时间正序。</li>'
					+'<li>全面优化域名绑定的设置弹窗。使之更容易理解、美观。</li>'
					+'<li>CMS模式网站管理后台中，增加欢迎页面，其内有当前用户信息、及常用功能快捷入口，同时增加常用快捷功能的指引，在这里进入某个功能时，左侧菜单自动定位到相应功能，以便让用户熟悉功能。登陆成功后默认进入此页。</li>'
					+'<li>去掉CMS模式网站管理后台中，帮助说明下的 模版使用 二级导航。</li>'
					+'<li>CMS模式网站管理后台，左侧功能菜单，增加快速指引函数。</li>'
					+'<li>CMS模式网站管理后台，选择模版后，便进入欢迎页面</li>'
					+'<li>CMS模式网站管理后台，选择模版页面，第一步中，自己制作模版，点击查看模版制作入门后，不再是直接打开新网页，而是自动打开左侧帮助说明菜单，自动点击 模版开发 打开模版开发入门。以便用户后续知晓模版入门在哪。</li>'
					+'<li>CMS模式网站管理后台，模版选择页面，优化顶部提示文字 “ 注意，当您使用了一个模版后，将无法再使用其他模版！如果要更换模版，可以再重新建立一个网站。 ”</li>'
					+'<li>优化网站管理后台，栏目管理，栏目列表调整为一页显示1000个栏目，也就是相当于在一页将所有栏目都显示出来。</li>'
					+'<li>优化CMS网站管理后台，内容管理中，当点击内容管理时，默认显示所有信息，将所有列出的信息，显示删除、转移栏目按钮。但是删除独立页面的栏目下的信息时，会提示独立页面的信息无法删除</li>'
					+'<li>修复CMS模式网站管理后台，第一次登陆，选择模版后，内容管理中编辑、添加信息失败的问题。</li>'
					+'<li>修复总管理后台，权限管理 - 资源管理，针对某个资源修改，或者添加资源时，一直转圈等待中，无响应的问题。</li>'
					+'<li>优化总管理后台，权限管理 - 资源管理 ，删除某个资源时，是否确认删除的提示框滑到右侧被遮挡的问题。</li>'
					+'<li>修复CMS模式网站管理后台，刚创建的网站，导入模版时，多次点击可以导入多次模版的问题。</li>'
					+'<li>修复CMS模式网站管理后台，刚创建的网站，利用导入本地模版文件快速创建网站，若导入失败，依旧提示成功的问题。</li>'
					+'<li>优化网站的三种模式，电脑端、手机端 这两种增加旧版标示，已不推荐使用。以后统一推荐使用新版CMS模式。</li>'
					+'<li>总管理后台 - 用户管理 ，去除删除用户的功能，以避免误操作删除一些主要的账号。</li>'
					+'<li>总管理后台，用户管理，用户列表中，底部增加说明提示，引导第一次使用的朋友，开通代理及网站，使用代理平台进行。</li>'
