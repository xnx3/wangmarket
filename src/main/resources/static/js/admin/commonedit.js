		...
		if(version.length > 0 && oldVersion != version){
			...
			layer.open({
				...
				,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300; text-align:left;">'
					...
					+'<li>代理后台，增加 自助建站 功能。代理也可以有自己的独立自助开通网站通道。通过此开通的网站，不消耗站币！</li>'
					+'<li>修复 UEditor 编辑器中，插入动态地图后，网站浏览时提示登录的问题。</li>'
					+'<li>优化 commonedit.js 、 fun.js 等js有的地方依旧调用cdn的情况，改为调用自己项目的。</li>'
					+'<li>优化开通网站时，手机、电脑模式过时的提示，去掉过时提示。因4.0版本就已经全部优化过了。</li>'
					+'<li>去除开通网站时，手机、电脑模式后面跟随的旧版本，不推荐使用的提示。</li>'
					+'<li>网站管理后台中，新版本更新提示，变为延迟3秒后弹出。以避免网络状况不好的情况下，异步加载js顺序靠后的问题。</li>'
					+'<li>总管理后台中，去掉对用户的角色更改。</li>'
					+'<li>修复在更改密码后，不出现结果提示的问题。</li>'
					+'<li>代理后台网站管理中，最底部 冻结 添加生效时间的说明</li>'
					+'<li>domain项目中，MQ信息同步增加map历史清楚，每隔24小时清除一次。</li>'
					+'<li>新建立的CMS类型网站，第一次登陆选择模版时，方式一自行建立模版，入门链接更改为最新的入门引导 <a href="http://tag.wscso.com/4192.html" target="_black" style="text-decoration: underline; color:#999">http://tag.wscso.com/4192.html</a></li>'
