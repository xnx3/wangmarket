		...
		if(version.length > 0 && oldVersion != version){
			...
			layer.open({
				...
				,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300; text-align:left;">'
					...
					+'<li>增加列表页模版中，文章列表调用，除了支持文章标签外，还支持栏目标签。调出的栏目属性为当前文章的所在的栏目。</li>'
					+'<li>图集news_extend_photos.js，增加photosHaveValue() 判断图集是否有值</li>'
					+'<li>云端模版更新接口，增加版本号的传递</li>'
					+'<li>sqlite数据库，优化template表主键及自增</li>'
					+'<li>template 数据表增加 resource_import 字段，可指定模版所用资源使用云端还是本地私有</li>'
					+'<li>commons-fileupload升级到1.3.3</li>'
					+'<li>修复功能插件-模版开发 ，当先建立模版页面、模版变量，再给模版起名时，模版变量跟模版页面关联不上当前模版名的问题。</li>'
					+'<li>修复属性由可视化模式，变为纯代码模式时，html模版中，引入资源路径多了一层文件夹的问题。</li>'
					+'<li>修复Windows系统中制作的模版文件，在Linux系统中无法使用的问题。</li>'
					+'<li>修复最新版模版开发时，可视化模式调出路径少文件夹的问题</li>'
					+'<li>修复生成整站时有的空指针bug</li>'
					+'<li>修复代理平台不弹出升级提示的bug</li>'
