...
$(function(){
	...
	var help_client_text = '要创建何种类型的网站。创建后不可更改！'+
					'<br/><b>1.&nbsp;CMS</b>&nbsp;（最新升级，推荐使用此种）全能模式！且百分百可自定义！类似于现有的织梦CMS、帝国CMS，懂HTML可以进行定制开发自己的网站，也可以根据导入模版一键创建出成品网站，只需要修改一下栏目名字、文字及图片就可达到使用级别！但是一旦导入模版使用后，就不可以再导入其他模版。此种类型因自由度极高，被广大建站爱好者青睐，其可以根据自己的意愿作出任意形式的网站。建议稍微懂点建站的，都可以试试这种模式！'+
					'<br/><b>2.&nbsp;电脑端</b>&nbsp;（版本太旧，已不推荐使用）最适合做SEO优化，但网站模版稍显单一。不过此种类型的网站有几套模版可以在手机或电脑中都可使用，且支持随意切换模版。推荐做SEO的网站首选。'+
					'<br/><b>3.&nbsp;手机端</b>&nbsp;（版本太旧，已不推荐使用）专为手机设计的网站，仅有手机页面，但手机体验也是最好的。支持随意切换的手机模版。';
