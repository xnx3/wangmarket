...
<h5>文章链接与自定义 HTML 名称</h5><p>搜索结果模板推荐使用 <strong>news.url</strong> 作为文章链接：它会自动生成当前文章实际使用的 HTML 页面地址。文章在内容管理中填写了自定义 HTML 名称时，例如填写 <strong>world</strong>，<strong>news.url</strong> 生成的链接就是 <strong>world.html</strong>；未填写时会自动回退为文章 ID，例如 <strong>123.html</strong>。</p><p><strong>news.htmlName</strong> 是文章设置的自定义 HTML 名称（不含 <strong>.html</strong> 后缀），未设置时为空；<strong>news.id</strong> 仅表示文章 ID。站内搜索接口返回的对应字段名为 <strong>html_name</strong>，搜索结果链接请使用 <strong>news.url</strong>。</p><pre class="prettyprint"><code>&lt;a href="news.url" target="_black" class="xnx3_news_item"&gt;news.title [news.addtime]&lt;/a&gt;</code></pre><p>旧模板中使用 <strong>news.id.html</strong> 的写法仍然兼容，系统会将其替换为文章实际页面名称；新建或修改模板时建议统一使用 <strong>news.url</strong>，以便同时支持自定义名称和未设置时的 ID 回退。</p>
...
<td>id</td>
<td>整数</td>
<td>信息编号，<strong>news.id</strong> 仅表示文章 ID；文章链接请使用 <strong>news.url</strong>，它会自动使用自定义 HTML 名称，未设置时回退为 ID（例如 123.html）</td>
</tr>
<tr>
<td></td>
<td>title</td>
