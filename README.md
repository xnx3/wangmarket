...
如果你要把当前项目交给第三方继续维护，或者想直接让大模型在仓库里进行二次开发，可将仓库根目录下的 `skills/` 整个目录一并交付。

1. 将 `skills/` 放到对方项目根目录，保持路径 `skills/wangmarket-dev/SKILL.md` 不变。
2. 这个 skill 面向当前 `wangmarket` 仓库，或保留了同样目录结构的直接派生项目。
3. 大模型进入仓库后，会优先参考 `skills/wangmarket-dev/` 下的仓库结构、改动工作流、官方文档索引，再决定改动落点。
4. 这个本地 skill 明确排除了 `AI-SEO` 产品文档，只覆盖网市场本体的安装、模板、插件与二次开发。
5. 如果对方项目已经大幅调整目录结构，请先同步更新 `skills/wangmarket-dev/references/repo-architecture.md`，再让大模型做大范围改动。
...
* **[自助开通网站](http://wang.market/plugin/phoneCreateSite/reg.do?inviteid=50)** ，用户可通过手机号跟验证码的方式自助开通网站，进行管理及使用网站。而这个过程我方可以无需任何人工参与。注意：这个入口来自另一个独立仓库项目，不属于当前 `wangmarket` 仓库的维护范围；如果你是在当前仓库里做二次开发或让大模型改代码，请忽略这一项。
...
* **[后台开放API](http://www.wang.market/35328.html)** ，对于后台操作，提供代理后台、网站管理后台的API接口。比如代理后台开通网站及管理网站、网站后台通过接口发布文章、生成整站等功能，都可通过API接口来进行。
...
* **[多语言并可分配子域名及收录](https://translate.zvo.cn/236896.html)** ，这个功能并非网市场本身的功能，而是我们另一款产品TCDN的能力，只不过两者可以非常完美的结合使用。它可以翻译为上百个其他语种，并且每个语种分配相关的子域名，翻译后的语种支持搜索引擎收录和排名。
...
[点此查看各种部署方式及步骤说明](http://cms.zvo.cn/40827.html)
